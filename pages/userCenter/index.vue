  ...
  <view class="container">
    ...
    <view class="custom-header" :style="headerStyle">
      <view class="header-content">
        <image class="logo" :src="logoSrc" mode="widthFix" />
        <view class="user-info" v-if="account || mobile">
    ...
    <scroll-view class="content" scroll-y :style="contentStyle">
      <view class="list-container">
        <view 
          ...
          :key="item.handleEvent" 
          :class="{ last: index === contentList.length - 1 }"
...
export default {
  ...
  computed: {
    // 计算属性优化样式绑定
    headerStyle() {
      const { statusBarHeight, navBarHeight, capsuleRight } = this.headerConfig;
      return {
  ...
  methods: {
    ...
    getSystemInfo() {
      ...
      const { statusBarHeight, screenWidth, platform, miniProgram } = this.systemInfo;
    ...
    handleItemClick(item) {
      const actionMap = {
        ...
        switchAccount: () => uni.navigateTo({ url: '/pages/login/index' }),
        signOut: () => this.logout()
      };
  ...
  .list-container {
    ...
    .list-item {
      ...
      .item-left {
        ...
        .item-icon { width: 30rpx; height: 30rpx; }
        .item-text {
          font-size: 28rpx;
          font-weight: bold;
          color: #333;
      ...
      .arrow-icon { width: 25rpx; height: 25rpx; }
    }
  }
}
</style>